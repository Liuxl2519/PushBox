...
// 规则
var ruleText = [
"1.	选择角色（默认为Girl）及难度（默认为简单），开始游戏。",
"2.	通过键盘上下左右键控制角色移动，箱子后方无障碍物时可被推动，",
"地图中全部箱子均被推到蓝色圆点上即为通关。",
"3. 通关等级根据完成关卡所用的步数划分（Excellent,Great,Good）。" ,         
"4.	游戏中获得的金币可用于重玩关卡（需2枚）、随机抽取关卡（需3枚）",
"及直接通关（需5枚），通关等级为Excellent也可获得金币。",
"5.祝你好运~"
]
...
var block, wall, box, ball, up, down, left, right,
	jb, temp1, temp2, diploma, gameover, box1, yx, rule,rulebg,excellent,great,good,end;
...
function gamePg() {
	keyboard = true;
	//flag = 1;
	if(chooseLevel[1].checked === true) { // 中等
		iCurlevel = 3;
		levelFlag = 1;
	}
	else if(chooseLevel[2].checked === true) { // 困难
		iCurlevel = 6;
		levelFlag = 2;
	}
	else  // 简单
...
function ruleBg() { // 规则显示
	context.drawImage(rulebg, 0, 0, rule.width * 0.75, rule.height * 0.75);
...
function initLevel() {
	...
	returnstart.style.display = 'block';
	next.style.display = 'block';
	replay.style.display = 'block';
	jbk5.style.display = 'block';
}
...
function gameSuccess(x, y) {  // 视玩家步数显示不同等级奖状
	...
	if(x <= y * 1.1) {
		...
		context.drawImage(excellent, 0, 0, canvas.width,canvas.height);
		...
		context.drawImage(good, 0, 0, canvas.width,canvas.height);
...
function drawImage(y) {  // 游戏结束（失败及全通关）标志
	...
	context.drawImage(yx, 60, y - 550, 480, 480);
}
