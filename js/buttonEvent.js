...
random.onclick = function(e) {
	if(JB >= 3) {
		...
		paused = false;
		context.clearRect(0, 0, canvas.width, canvas.height);
		animateButton.style.display = "block";//stop按钮
		returnstart.style.display = 'none';
		next.style.display = 'none';
		replay.style.display = 'none';
		jbk5.style.display = 'none';
...
returnstart.onclick = function(e) {
	...
	context.drawImage(end, 0, 0, canvas.width,canvas.height);
	/*context.font = "25px Microsoft YaHei";
	context.fillText("继续努力呦~", 100, 150);
	context.fillText("加油！", 100, 250);
	context.fillText("点击下面的按钮返回!", 100, 350);*/
